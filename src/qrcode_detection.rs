...
impl ObjectSubclass for QRCodeDetector {
    ...
    fn class_init(klass: &mut subclass::simple::ClassStruct<Self>) {
        ...
        klass.set_metadata(
            ...
            "Inference",
            "QRCodeDetector",
            "Raúl Pleitez <racsoraul@gmail.com>",
        );
...
impl BaseTransformImpl for QRCodeDetector {
    ...
    fn transform(
        ...
        if out_format == gst_video::VideoFormat::Rgb {
            ...
            let _decoded_data = detector.detect_and_decode(&src, &mut pts, &mut straight).unwrap();
            ...
            if !straight.empty().unwrap() {
                ...
                // println!("{:?} - Decoded Data: {}", p_to_slice, _decoded_data);
            }
...
// the name "qrcode_detector" for being able to instantiate it via e.g.
